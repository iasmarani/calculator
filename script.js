...
let btns = document.querySelectorAll('.splitButton')


// Get number of people from number of people div
let peopleNumber = Number(numberOfPeople.textContent);
...
const calculateBill = () => {
  ...
  let tipAmount = billInput * tip
  // console.log({tipAmount})

  // calculate the total (tip amount + bill)
  let total = tipAmount + billInput

  // calculate the per person total (total divided by number of people)
  let totalPerson = total / peopleNumber;

  // update the perPersonTotal on DOM & show it to user
  perPersonTotal.textContent = `Rs.${totalPerson.toFixed(2)}`;
}
...
if(peopleNumber === 1) {
//  throw alert('You are not allowed to divide bill less than 1 person') 
  return
}
